...
# Example data
# Let's create a matrix of random data
set.seed(123) # For reproducibility
data_matrix <- matrix(rnorm(100), nrow=10)

# Adding row and column names for illustration
rownames(data_matrix) <- paste("Gene", 1:10)
colnames(data_matrix) <- paste("Sample", 1:10)

# Generate the heatmap
# scale = "row" will scale the data in each row to have mean 0 and sd 1.
# clustering_distance_rows and clustering_distance_cols define the distance measures for row and column clustering. "euclidean" is the default.
# clustering_method defines the agglomeration method for clustering, "complete" is an example.
pheatmap(data_matrix,
         ...
         clustering_distance_rows = "none", # Do not cluster rows
         cluster_cols = TRUE, # Enable clustering for columns (samples)
         cluster_rows = FALSE, # Set to TRUE if you also want to cluster rows (fea
         main = "Heatmap with Hierarchical Clustering")
